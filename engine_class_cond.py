...
def train_one_epoch(model: torch.nn.Module, ae: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (points, labels, surface, categories,_) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
...
def evaluate(data_loader, model, ae, criterion, device):
    ...
    for points, labels, surface, categories,_ in metric_logger.log_every(data_loader, 50, header):
