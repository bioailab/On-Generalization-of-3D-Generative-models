...
def edm_sampler(
    ...
    for path in class_labels:
        image = Image.open(path).convert('RGB')
        image_tensor = transform(image).unsqueeze(0)  # Add batch dimension
        image_tensor = image_tensor.to(latents.device, non_blocking=True)
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        # Euler step.
        denoised = net(x_hat, t_hat, image_tensors).to(torch.float64)
        d_cur = (x_hat - denoised) / t_hat
        x_next = x_hat + (t_next - t_hat) * d_cur

        # Apply 2nd order correction.
        if i < num_steps - 1:
            denoised = net(x_next, t_next, image_tensors).to(torch.float64)
            d_prime = (x_next - denoised) / t_next
            x_next = x_hat + (t_next - t_hat) * (0.5 * d_cur + 0.5 * d_prime)
        # np.save("/home/workspace/3DShape2VecSet/Diffusion_step/embedding_{}.npy".format(i), x_next.cpu().numpy())
        # np.save("/home/workspace/3DShape2VecSet/Diffusion_step/embedding_{}.npy".format(i),x_next.cpu().numpy())
        # torch.save(x_next, "/home/workspace/3DShape2VecSet/Diffusion_step/embedding_{}.pt".format(i))
    return x_next
...
class StackedRandomGenerator:
    ...
    def randn(self, size, **kwargs):
        # print('*'*100)
        # print(size[0],len(self.generators))
        # print('*'*100)
...
class EDMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        # self.category_emb = nn.Embedding(55, n_heads * d_head)      -----> Class-Conditioned
        self.category_emb = models.resnet18(pretrained=True)
        
        # Replace the last fully connected layer
        # `emb_size` should be `n_heads * d_head` in your case
        num_features = self.category_emb.fc.in_features
        self.category_emb.fc = nn.Linear(num_features, n_heads * d_head)
        # print(n_heads * d_head)
    ...
    @torch.no_grad()
    def sample(self, cond, device,batch_seeds=None):
        # print(batch_seeds)
        if cond is not None:
            batch_size = len(cond)
...
def kl_d512_m512_l64_d24_edm():
    model = EDMPrecond(n_latents=512, channels=64, depth=24)
    return model
