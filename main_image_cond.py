...
from engine_image_cond import train_one_epoch, evaluate
...
def get_args_parser():
    ...
    parser.add_argument('--ae-pth', default='outputImageConditioned/ae/kl_d512_m512_l64/600-3_he_2/checkpoint.pth', help='Autoencoder checkpoint')

    parser.add_argument('--point_cloud_size', default=2048, type=int,
                        help='input size')
    parser.add_argument('--max_train_size', type=int, required=True,
                        help='max samples from a particular class')
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='/home/workspace/3DShape2VecSet/Dataset', type=str,
                        help='dataset path')
...
def main(args):
    ...
    model = models_image_cond.__dict__[args.model]()
    model.to(device)
    ...
    criterion = models_image_cond.__dict__['EDMLoss']()
    ...
    # Assuming train_losses are recorded every epoch
    train_epochs = range(1, args.epochs + 1)

    # Assuming val_losses are recorded every 5th epoch starting from the first epoch where validation occurs
    # Adjust this if your first validation occurs at a different epoch
    val_epochs = range(5, args.epochs + 1, 5)
    val_losses = val_losses[:-1] 
    plt.figure(figsize=(10, 5))
    plt.plot(train_epochs, train_losses, label='Training Loss')
    plt.plot(val_epochs, val_losses, label='Validation Loss', linestyle='--')  # Using a different line style for clarity
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training and Validation Loss Over Epochs')
    plt.xticks(list(plt.xticks()[0]) + [val_epochs[-1]])  # Ensure the last validation epoch is included as a tick
    plt.legend()
    ...
    plt.savefig(os.path.join(args.output_dir, 'loss_vs_epoch.png'))
    plt.show()
