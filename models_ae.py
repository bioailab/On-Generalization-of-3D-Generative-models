...
class DiagonalGaussianDistribution(object):
    ...
    # Wasserstein regularizer with other set to mean 0, var 1
    def wasserstein(self):
        if self.deterministic:
            return torch.Tensor([0.])
        else:
            # Mean and std of the Gaussian distribution
            mu = self.mean
            sigma = self.std

            # Calculate squared differences
            mean_diff_sq = torch.pow(mu, 2)  # (self.mean - 0)^2
            std_sum = torch.pow(sigma, 2) + 1  # Variance sum
            
            # Calculate Wasserstein distance
            wasserstein_distance = torch.sqrt(mean_diff_sq + std_sum - 2 * sigma)
            
            # Sum across dimensions if needed
            return torch.mean(wasserstein_distance, dim=[1, 2])

    # Hellinger regularizer with other set to mean 0, var 1
    def hellinger(self):
        if self.deterministic:
            return torch.Tensor([0.])
        # For the other distribution: mean=0, var=1
        term1 = torch.pow(self.mean, 2) / (4 * (self.var + 1))  # (self.mean - 0)^2 / (4 * (self.var + 1))
        term2 = (2 * self.std * 1) / (self.var + 1)  # (2 * self.std * 1) / (self.var + 1)
        hellinger_distance = 1 - torch.sqrt(term2) * torch.exp(-term1)
        return torch.mean(hellinger_distance, dim=[1, 2])
